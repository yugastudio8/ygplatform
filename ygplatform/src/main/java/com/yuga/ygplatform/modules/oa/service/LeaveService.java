 ...
 */
package com.yuga.ygplatform.modules.oa.service;
...
import com.yuga.ygplatform.common.utils.Collections3;
import com.yuga.ygplatform.modules.oa.dao.LeaveDao;
import com.yuga.ygplatform.modules.oa.entity.Leave;
import com.yuga.ygplatform.modules.sys.utils.UserUtils;
import com.yuga.ygplatform.modules.sys.utils.workflow.ProcessDefinitionKey;
...
public class LeaveService extends BaseService {
	...
	public Leave findOne(Long id) {
		...
		if (historicProcessInstance != null) {
			variables = Collections3.extractToMap(historyService.createHistoricVariableInstanceQuery()
					.processInstanceId(historicProcessInstance.getId()).list(), "variableName", "value");
		} else {
			variables = runtimeService.getVariables(runtimeService.createProcessInstanceQuery()
					.processInstanceId(leave.getProcessInstanceId()).active().singleResult().getId());
		}
	...
	public ProcessInstance save(Leave entity, Map<String, Object> variables) {
		...
		identityService.setAuthenticatedUserId(ObjectUtils.toString(1L));

		ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(ProcessDefinitionKey.Leave.getKey(),
				businessKey, variables);
		String processInstanceId = processInstance.getId();
		entity.setProcessInstanceId(processInstanceId);
		leaveDao.updateProcessInstanceId(entity.getId(), entity.getProcessInstanceId());
		logger.debug("start process of {key={}, bkey={}, pid={}, variables={}}", new Object[] {
				ProcessDefinitionKey.Leave.getKey(), businessKey, processInstanceId, variables });
	...
	public List<Leave> findTodoTasks(String userId) {
		...
		// 根据当前人的ID查询
		List<Task> todoList = taskService.createTaskQuery().processDefinitionKey(ProcessDefinitionKey.Leave.getKey())
				.taskAssignee(userId).active().orderByTaskPriority().desc().orderByTaskCreateTime().desc().list();
		// 根据当前人未签收的任务
		List<Task> unsignedTasks = taskService.createTaskQuery()
				.processDefinitionKey(ProcessDefinitionKey.Leave.getKey()).taskCandidateUser(userId).active()
		...
		for (Task task : tasks) {
			...
			leave.setProcessDefinition(repositoryService.createProcessDefinitionQuery()
	...
	public Page<Leave> find(Page<Leave> page, Leave leave) {
		...
		for (Leave item : result.getList()) {
			...
			if (historicProcessInstance != null) {
				...
				item.setProcessDefinition(repositoryService.createProcessDefinitionQuery()
